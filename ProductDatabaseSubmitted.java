...
public class ProductDatabase extends JFrame implements ActionListener
{
    ...
    public static void main(String[] args)
    {
        ...
        frame.setTitle("Product sales chart. Student no: 2716761");
        frame.createGUI();
    ...
    private void addNewProductAction()
    {
        ...
        //boolean ok = 
        addNewProduct(newName);  // Try to add, ok reports the result
        /**if (!ok)                           // Check for failure
         * Moved to the addNewProduct method because it can generate a different type of alert, 
         * if the text is already present.
   ...
   /**
    ...
    {
        //Goes through every product in the list and sets sales and percentage to 0.
    	for(int i = 1; i <= actualProducts; i++) {
    		productSales[i] = 0;
    		percentage[i] = 0;
    	}
    	totalSales = 0; //Total sales is set to 0.
    }
    ...
    {
        // Calculates every percentage, rounding it to the first decimal place.
    	for(int i = 1; i <= actualProducts; i++) {
    		if(totalSales != 0) {	//Avoid dividing by 0
    			percentage[i] = Math.round(1000 * productSales[i] / totalSales) / (float) 10;
    		} else{					//If totalSales = 0, all percentages are 0
    			percentage[i] = 0;
    		}
    	}
    }
    ...
    {
    	boolean status = true; 						   // Assume that adding is a success
        for(int i = 1; i <= actualProducts; i++) {	   // Check that the item isn't already in the list
        	if(productName[i].toLowerCase().equals(newName.toLowerCase())) {
        		JOptionPane.showMessageDialog(this, "The product " + newName + " already exists.");
        		status = false; 					   // Failure
        	}
        }
        if(status) {								   // If the item isn't already in the list
        	if(actualProducts < 10) {				   // Check that there are less than 10 products
    ...
    {
    	//Removes previous sales amount from the total and adds new one amount.
    	totalSales = totalSales - productSales[productNumber] + prodSales;
    	//Changes the sales amount in the productSales Array.
    ...
    {
    	totalSales -= productSales[deleteNumber];	//Removes current sales from total
    	//Moves the remaining items up the list, removing the deleted one
    	...
    	productName[actualProducts] = "";
        productSales[actualProducts] = 0;
        percentage[actualProducts] = 0;
        actualProducts --;	//Finally reduces the number of actual products
    	computePercentages();
    }

    /**
     * This method exchanges the place of two elements in the database.
     * Allows for use with different conditions, in this case by both sorting functions.
     * 
     * @param i	  the index of the current item being considered
     */
    private void sortUp(int i) {
    	// Initialising and declaring the temp variables to hold the smaller item.
		int tempSales = productSales[i];
		String tempName = productName[i];
		float tempPercentage = percentage[i];
		// The bigger item is moved up the list.
		productSales[i] = productSales[i+1];
		productName[i] = productName[i+1];
		percentage[i] = percentage[i+1];
		// The smaller item is moved down the list, out of the temp variables.
		productSales[i+1] = tempSales;
		productName[i+1] = tempName;
		percentage[i+1] = tempPercentage;
    }
    ...
    {
    	// Iterates through the sorting algorithm as many times as there are items.
    	for(int j = 0; j < actualProducts; j++) {
    		// Sorting algorithm that moves items closer to their intended position.
	    	for (int i = 1; i < actualProducts; i++){
